...
import { Target, TrendingUp, Zap, Clock, Hash, ChevronLeft, ChevronRight, Flame, Trophy, Brain, Layers } from 'lucide-react';
...
function StreakCalendar({ profileId }: { profileId: number }) {
  ...
  const { data: streakInfo } = useQuery({
    ...
    queryFn: () => customFetch<{ currentStreak: number; longestStreak: number; completedToday: boolean }>(
  ...
  return (
    <Card className="shadow-md border-orange-200/60">
      ...
      <CardContent className="space-y-3">
        <div className="flex items-center justify-between">
          <button onClick={() => setViewDate(d => new Date(d.getFullYear(), d.getMonth() - 1, 1))} disabled={isPrevDisabled} className="p-1 rounded hover:bg-muted disabled:opacity-30 transition-colors">
            <ChevronLeft className="w-4 h-4" />
          </button>
          <p className="text-sm font-semibold">{MONTH_NAMES[viewDate.getMonth()]} {viewDate.getFullYear()}</p>
          <button onClick={() => setViewDate(d => new Date(d.getFullYear(), d.getMonth() + 1, 1))} disabled={isNextDisabled} className="p-1 rounded hover:bg-muted disabled:opacity-30 transition-colors">
        ...
        <div className="grid grid-cols-7 gap-1">
          ...
          {Array.from({ length: daysInMonth }, (_, i) => {
            ...
            return (
              <div key={day} title={done ? `Completed ${dateStr}` : undefined}
                className={['aspect-square flex items-center justify-center rounded-full text-xs font-semibold transition-colors',
                  done ? 'bg-orange-500 text-white' : isToday ? 'ring-2 ring-orange-400 text-foreground' : isFuture ? 'text-muted-foreground/40' : 'text-muted-foreground',
                ].join(' ')}>
                {done ? '🔥' : day}
              </div>
            );
      ...
      <CardContent className="space-y-3">
        ...
        <div className="grid grid-cols-7 gap-1">
          ...
          {Array.from({ length: daysInMonth }, (_, i) => {
            ...
            const isFuture = dateStr > todayStr;
            return (
              <div key={day} title={done ? `Played on ${dateStr}` : undefined}
                className={['aspect-square flex items-center justify-center rounded-full text-xs font-semibold transition-colors',
                  done ? 'bg-purple-500 text-white' : isToday ? 'ring-2 ring-purple-400 text-foreground' : isFuture ? 'text-muted-foreground/40' : 'text-muted-foreground',
                ].join(' ')}>
                {done ? '🃏' : day}
              </div>
            );
          ...
          <Card className="shadow-md border-primary/10">
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                <Clock className="w-5 h-5 text-primary" /> Best Times by Difficulty
              </CardTitle>
            </CardHeader>
            <CardContent>
      ...
      {mode === 'memory' && (
        <div className="space-y-6 animate-in fade-in duration-300">
          ...
          <Card className="shadow-md border-purple-200/60">
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                <Clock className="w-5 h-5 text-purple-500" /> Best Times by Grid Size
              </CardTitle>
            </CardHeader>
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">

...
export function BadgeShareSheet({
  ...
  const platforms = [
    ...
    },
    {
      id: "facebook",
      label: "Facebook",
      color: "bg-[#1877F2] hover:bg-[#1469d6] text-white",
      icon: (
        <svg viewBox="0 0 24 24" className="w-5 h-5 fill-current" aria-hidden>
    ...
    },
    {
      id: "telegram",
      label: "Telegram",
      color: "bg-[#26A5E4] hover:bg-[#1d96d4] text-white",
      icon: (
        <svg viewBox="0 0 24 24" className="w-5 h-5 fill-current" aria-hidden>
  ...
  return (
    <Dialog open={open} onOpenChange={(o) => { if (!o) onClose(); }}>
      <DialogContent className="max-w-sm">
        <DialogHeader>
          <DialogTitle className="text-base">Share your badge</DialogTitle>
        </DialogHeader>

        {/* Preview blurb */}
        <div className="rounded-lg bg-muted/60 px-4 py-3 text-sm text-muted-foreground leading-relaxed whitespace-pre-line">
          {text}
        </div>
        ...
        <div className="grid grid-cols-2 gap-2 mt-1">
          {platforms.map((p) => (
            <button
              ...
              className={`flex items-center gap-2.5 px-4 py-2.5 rounded-xl text-sm font-semibold transition-all active:scale-95 ${p.color}`}
            >
              {p.icon}
              {p.label}
            </button>
        ...
        <p className="text-[11px] text-muted-foreground text-center -mt-1">
          For Instagram — copy the link below and paste it in your story or bio.
        </p>

        {/* Copy link row */}
        <div className="flex items-center gap-2 rounded-lg border bg-muted/40 px-3 py-2">
          <span className="flex-1 text-xs truncate text-muted-foreground font-mono">{shareUrl}</span>
          <Button size="sm" variant="ghost" className="h-7 gap-1.5 text-xs shrink-0" onClick={handleCopy}>
